...
use syn::{parse_macro_input, DeriveInput, Path, parse::Parse, Token};
...
impl Parse for EventBasedStateStruct {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        Ok(EventBasedStateStruct { event_type, process_function })
    }
}
...
pub fn derive_states(input: TokenStream) -> TokenStream {
    ...
    let event_based: Option<EventBasedStateStruct> = ast.attrs.iter().find_map(|v| {
        match &v.meta {
            syn::Meta::List(list) =>  {
    ...
    let event_based_tokens = if let Some(EventBasedStateStruct { event_type, process_function }) = event_based {
        let mut event_based_trait = base_trait_path.clone();
        event_based_trait.segments.push(format_ident!("EventBasedState").into());
